...
export function JournalDetail({ entry }) {
  ...
  const totalDebit = entry.totalDebit ?? entry.lines?.reduce((s, l) => s + (Number(l.debit) || 0), 0) ?? 0
  const totalCredit = entry.totalCredit ?? entry.lines?.reduce((s, l) => s + (Number(l.credit) || 0), 0) ?? 0
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-1">
          ...
          {entry.reference && (
            <div className="text-xs text-text-muted">
              {t('journal.reference')}: <span className="text-text-secondary font-medium">{entry.reference}</span>
            </div>
          )}
        </div>
      ...
      {/* ── Reversal notice ── */}
      {(entry.originalEntryId || entry.reversedEntryId) && (
        <div className="flex items-start gap-2 px-3 py-2.5 rounded-md bg-info-soft border border-info/20 text-xs text-info">
          ...
          <span>
            {entry.originalEntryId
              ? `${t('journal.reversalOf')}: ${entry.originalEntry?.entryNumber || entry.originalEntryId}`
              : `${t('journal.reversedBy')}: ${entry.reversedEntry?.entryNumber || entry.reversedEntryId}`}
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
        {entry.createdBy && (
          <div className="text-xs text-text-muted">
            <span className="block text-text-secondary font-medium mb-0.5">{t('common.createdAt')}</span>
            {formatDateTime(entry.createdAt, i18n.language)}
            {entry.createdBy?.fullName && (
              <span className="text-text-primary"> · {entry.createdBy.fullName}</span>
            )}
          </div>
        )}
        {entry.postedAt && (
          <div className="text-xs text-text-muted">
            <span className="block text-text-secondary font-medium mb-0.5">{t('journal.postedAt')}</span>
            {formatDateTime(entry.postedAt, i18n.language)}
          </div>
        )}
        {entry.updatedAt && entry.updatedAt !== entry.createdAt && (
          <div className="text-xs text-text-muted">
            <span className="block text-text-secondary font-medium mb-0.5">{t('common.updatedAt')}</span>
