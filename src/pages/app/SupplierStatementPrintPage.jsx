...
async function getPrintableSupplierStatement(id) {
  const firstPage = await supplierApi
  ...
  return (
    <span
      className={[
        ...
        type === 'bill' ? 'bg-primary/10 text-primary-700' : 'bg-emerald-500/10 text-emerald-700',
      ].join(' ')}
    >
      {label}
    </span>
...
export default function SupplierStatementPrintPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['suppliers', 'statement-print', id],
    queryFn: () => getPrintableSupplierStatement(id),
    enabled: !!id,
  })

  useEffect(() => {
    const style = document.createElement('style')
    style.id = 'supplier-statement-print-styles'
    style.textContent = `
  ...
  return (
    <div dir={dir} lang={i18n.language} className="min-h-screen bg-gray-100 print:bg-white font-sans">
      <div className="print:hidden sticky top-0 z-10 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between shadow-sm">
        <Link
          to={ROUTES.SUPPLIER_STATEMENT(id)}
          className="flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900 transition-colors"
        >
          {isRtl ? <ArrowRight size={15} /> : <ArrowLeft size={15} />}
          {t('suppliers.printBack')}
        </Link>
        <button
        ...
        <div className="px-16 py-14 print:p-0">
          <div className="print-keep-together flex justify-between items-start mb-12 gap-8">
            ...
            <div className="text-end">
              ...
              <p className="text-sm font-semibold text-gray-500 mt-3 tracking-wide">{supplier.name}</p>
            </div>
          </div>
          ...
          <div className="print-keep-together flex justify-between items-start gap-8 mb-10">
            <div>
              ...
              <p className="text-base font-semibold text-gray-900 leading-tight">{supplier.name}</p>
              {supplier.email && <p className="text-sm text-gray-500 mt-0.5">{supplier.email}</p>}
              {supplier.phone && <p className="text-sm text-gray-500 mt-0.5">{supplier.phone}</p>}
              {supplier.address && <p className="text-sm text-gray-500 mt-0.5">{supplier.address}</p>}
            </div>
            <div className="space-y-2 text-end">
              <div className="flex items-center justify-end gap-5">
                <span className="text-[11px] text-gray-400">{t('suppliers.printDate')}</span>
              ...
              <div className="flex items-center justify-end gap-5">
                <span className="text-[11px] text-gray-400">{t('suppliers.outstandingBalance')}</span>
          ...
          {transactions.length === 0 ? (
            <div className="print-keep-together border border-dashed border-gray-300 rounded-2xl px-6 py-10 text-center text-sm text-gray-500">
              {t('suppliers.statementEmptyDesc')}
            </div>
          ) : (
            <table className="print-table w-full text-sm">
              ...
              <tbody>
                {transactions.map((transaction, index) => (
                  <tr
                    key={transaction.journalEntryId || `${transaction.type}-${transaction.billId}-${transaction.date}-${index}`}
                  ...
                  >
                    ...
                    <td className="py-3">
                      <TransactionTypeLabel
                        ...
                        label={transaction.type === 'bill' ? t('suppliers.statementBill') : t('suppliers.statementPayment')}
                      />
                    </td>
