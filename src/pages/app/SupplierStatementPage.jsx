  ...
  return (
    <div className="space-y-6">
      ...
      {transactions.length === 0 ? (
        <EmptyState
          title={t('suppliers.statementEmptyTitle')}
          description={t('suppliers.statementEmptyDesc')}
        />
      ) : (
        <Card>
          <div className="px-4 py-3 border-b border-border bg-surface-subtle">
            <h3 className="text-sm font-semibold text-text-primary">{t('suppliers.statementTable')}</h3>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-surface-subtle text-text-muted text-xs font-semibold uppercase tracking-wide">
                  ...
                  <th className="px-4 py-3 text-start">{t('suppliers.transactionType')}</th>
                  <th className="px-4 py-3 text-start">{t('suppliers.reference')}</th>
                ...
                {transactions.map((transaction, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-end">
                      {transaction.billId ? (
                        <Link
                          to={ROUTES.BILL_DETAIL(transaction.billId)}
