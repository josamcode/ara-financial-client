...
import { useAPAging } from '@/features/reports/hooks/useReports'
...
export default function APAgingPage() {
  ...
  const query = useAPAging(appliedParams)
  ...
  return (
    <div className="animate-fade-in space-y-4">
      ...
      <PageHeader
        ...
        subtitle={t('reports.apAgingSubtitle')}
      />
      ...
      {!query.isLoading && !query.isError && hasData && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            />
            <SummaryCard
              label={t('reports.suppliersWithOutstanding')}
              value={formatNumber(summary?.suppliersWithOutstanding, locale, 0)}
            />
            <SummaryCard
              label={t('reports.overdueBillsCount')}
              value={formatNumber(summary?.overdueBillsCount, locale, 0)}
            />
          </div>
              ...
              <table className="w-full text-sm min-w-[52rem]">
                <thead>
                  <tr className="border-b border-border bg-surface-subtle text-text-muted text-xs font-semibold uppercase tracking-wide">
                    <th className="px-4 py-3 text-start">{t('bills.supplier')}</th>
                  ...
                  {rows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-end">
                        {row.supplierId ? (
                          <Link
                            to={ROUTES.SUPPLIER_STATEMENT(row.supplierId)}
